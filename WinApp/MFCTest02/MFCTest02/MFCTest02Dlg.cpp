...
BEGIN_MESSAGE_MAP(CMFCTest02Dlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDYES, &TestFunc)
	ON_BN_CLICKED(IDOK, &CMFCTest02Dlg::OnBnClickedOk)
...
CString ToUpper(CString s)	//소문자 ==> 대문자로   xx1x xxxx  ==>  xx0x xxxx
							//					   1101 1111  DFh (&)
{
	CString t; 
...
CString ToLower(CString s)	//대문자 ==> 소문자로   xx0x xxxx  ==>  xx1x xxxx
							//					   0010 0000  20h (|)
{
	CString t; 
	for (int i = 0; i < s.GetLength(); i++)
...
void CMFCTest02Dlg::OnBnClickedOk()
{
